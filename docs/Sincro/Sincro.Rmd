...
#Climatelabs <- read_excel("Multicriteria.xlsx") # before
Climatelabs <- read_excel("Multicriteria-Dec.xlsx") 
...
> Cette présentation fait un anlayse du Sinchro pour le Projet Climatelabs
...
```{r, include=TRUE}
knitr::include_graphics("Planning/planning.pdf")
```

![Image Title](Planning/planning.jpg)
...
Climatelabs.original %>% select(WP, Role, Unit.cost, Jours.GA, Gantt, Sinchro) %>% 
  set_names("WP", "Role", "Coût de chaque Rôle<br/>(Euros/Jour)", "Jours<br/>(G.A.)", "Jours<br/>(Equivalent EC)", "Gannt<br/>(Mois per WP)") %>% 
